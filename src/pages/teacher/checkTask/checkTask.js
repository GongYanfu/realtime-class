...
const { Option } = Select
const classworkTypes = ['课后作业','课堂作业']
...
class TaskTable extends Component {
    ...
    handleFindByCourseName = async() => {
        ...
        if(courseName && classworks){
            const newData = classworks.filter(item => item.courseName.match(courseName))
    ...
    handleFindByClassworkType = async() => {
        const { classworkType, classworks } = this.state
        if(classworkType && classworks){
            const newData = classworks.filter(item => item.classworkType.match(classworkType))
            if(newData && newData.length>0){
                this.setState({
                    classworks: newData
                })
    ...
    handleToCorrect = (record,index) => {
        this.setState({
            currentRecord: record
        })
        const currentClasswork = JSON.stringify(record)
        setItem(KEYS.KEY_CUR_CLASSWORK,currentClasswork)
    }
    ...
    render(){
        const { classworks, courses } = this.state
        const columns = [
            {
        ...
        return (
            <BasicLayoutForTea title='作业管理/作业批改' describe='本页列出所有作业并且可选择某一作业查看提交情况' >
                {

    ...
    loadAllHomeworks = async() => {
        ...
        if(allCourses && allClassworks){
            ...
            if(newData.length === allClassworks.length){
                homeworks = newData.filter(item => item.classworkType === '课后作业')
                if(homeworks.length===0){
                    message.info('暂无课后作业！！！',2)
                }
            }
        }
        if(homeworks && homeworks.length>0){
            ...
            if(IDs.length === homeworks.length){
                    ...
                    if(res && res.status===200){
                        ...
                        if(allAnswers.length === IDs.length){
                            homeworks.map((item1,index1) =>{
                                ...
                                if(classworks.length === homeworks.length){
                                    this.setState({
                                        classworks
                                    })
                                }
    ...
    render(){
        ...
        const columns = [
            ...
            },
            {
                ...
                dataIndex:'state',
                key:'state',
                filters:[{text:'提交一次',value:'answerFirstImgPath'},{text:'提交两次',value:'answerSecondImgPath'}],
                filteredValue: filteredInfo.state || null,
                onFilter:(value,record) =>  value==='answerFirstImgPath' ? record[value] && !record['answerSecondImgPath'] : record[value] ,
        ...
        return (
            <BasicLayoutForStu title={'作业管理/课后作业'} describe='本页用于展示所有课后作业'>
                {
                    /* classworks && classworks.length
                    ? */   <Fragment>
                            <Form 
                                layout='inline' 
                                style={{
                ...
                }
                {
                    upload_visible && <Modal
                                        ...
                                        onCancel={this.handleCancel}
                                    >
                                        <UploadForm record={currentRecord} onCancel={this.handleCancel} onLoad={this.loadAllHomeworks} />
                                    </Modal>
                }
            </BasicLayoutForStu>
        )
